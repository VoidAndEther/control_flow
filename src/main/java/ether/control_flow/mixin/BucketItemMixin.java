    ...
    implements BucketItemAccessor
{
    ...
    @WrapOperation(
        ...
        at = @At(
            value = "INVOKE",
            target = "Lnet/minecraft/block/FluidDrainable;getBucketFillSound()Ljava/util/Optional;"
        )
    )
    private Optional<SoundEvent> redirect_sound(
        FluidDrainable instance,
        Operation<Optional<SoundEvent>> original,
        @Local(name = "itemStack2") ItemStack filledBucket
    ) {
        return ((BucketItemAccessor) filledBucket.getItem())
            .getFluid()
            .getBucketFillSound();
    }
}
